...
static int string_to_hex(char *str, unsigned char *out, unsigned int *outlen)
{
	char *p = str;
	char high = 0, low = 0;
				     ...
				     struct device_attribute *attr, const char *buf, size_t size)
{
	...
	string_to_hex(buffer, data, &count);
	if (count > alloc_size) {
		class_err("%s: string_to_hex produced too many bytes (%u), clamping to %zu\n",
