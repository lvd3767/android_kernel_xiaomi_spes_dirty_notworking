...
static inline int pd_handle_tcp_event_pr_swap(struct pd_port *pd_port, u8 new_role)
{
...
static inline int pd_handle_tcp_event_dr_swap(struct pd_port *pd_port, u8 new_role)
{
...
static inline int pd_handle_tcp_event_vconn_swap(struct pd_port *pd_port, u8 new_role)
{
...
static inline int pd_handle_tcp_event_request(struct pd_port *pd_port)
{
	...
	case TCP_DPM_EVT_REQUEST:
		ret = pd_dpm_update_tcp_request(pd_port,
						&tcp_event->tcp_dpm_data.pd_req);
		break;
...
static inline int pd_make_tcp_event_transit_ready(struct pd_port *pd_port, u8 state)
{
	if (!pd_check_pe_state_ready(pd_port))
		return TCP_DPM_RET_DENIED_NOT_READY;
