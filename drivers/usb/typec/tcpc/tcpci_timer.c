...
static inline u64 tcpc_get_timer_enable_mask(struct tcpc_device *tcpc)
{
	u64 data;
	unsigned long flags;
...
static inline u64 tcpc_get_timer_tick(struct tcpc_device *tcpc)
{
	u64 data;
	unsigned long flags;
...
#ifdef CONFIG_USB_POWER_DELIVERY
static inline void on_pe_timer_timeout(struct tcpc_device *tcpc, u32 timer_id)
{
...
{
	int i = 0;
	u64 triggered_timer = tcpc_get_timer_tick(tcpc);
	u64 enable_mask = tcpc_get_timer_enable_mask(tcpc);
...
static int tcpc_timer_thread_fn(void *data)
{
	...
	while (true) {
		wait_event(tcpc->timer_wait_que,
			   tcpc_get_timer_tick(tcpc) || kthread_should_stop());
