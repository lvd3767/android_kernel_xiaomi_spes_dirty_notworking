...
/* =================================================== */
/* ADDITIONAL STUBS FOR MultiSU/sidex15 COMPAT         */
/* SID globals are in MultiSU/kernel/selinux/selinux.c */
/* =================================================== */
...
static bool current_proc_umounted = false;
struct work_struct susfs_extra_works;

bool susfs_is_current_proc_umounted(void) { return current_proc_umounted; }
void susfs_set_current_proc_umounted(void) { current_proc_umounted = true; }
...
int susfs_add_sus_memfd(void __user *user_info) { return 0; }
...
struct susfs_handle_setuid_tw { struct callback_head cb; uid_t uid; };
static __maybe_unused void susfs_handle_setuid_tw_func(struct callback_head *cb) {
	struct susfs_handle_setuid_tw *tw = container_of(cb, struct susfs_handle_setuid_tw, cb);
	kfree(tw);
}
static __maybe_unused void ksu_handle_extra_susfs_work(void) {
	struct susfs_handle_setuid_tw *tw = kzalloc(sizeof(*tw), GFP_ATOMIC);
	if (!tw) return;
	if (work_pending(&susfs_extra_works)) return;
	tw->uid = current_uid().val;
	task_work_add(current, &tw->cb, true);
}

/* Extra stubs for missing symbols */
void susfs_try_umount_all(void) { }
int susfs_run_sus_path_loop(void __user *user_info) { return 0; }

/* ksu_try_umount - declared extern in susfs v1.5.5, must be defined */
void ksu_try_umount(const char *mnt, bool check_mnt, int flags, uid_t uid) {
	/* stub: actual implementation would iterate mounts and umount */
}
