...
static inline void print_ctrl_msg_event(struct tcpc_device *tcpc, u8 msg)
{
...
static inline void print_data_msg_event(struct tcpc_device *tcpc, u8 msg)
{
...
static inline void print_ext_msg_event(struct tcpc_device *tcpc, u8 msg)
{
...
static inline void print_hw_msg_event(struct tcpc_device *tcpc, u8 msg)
{
	if (msg < PD_HW_MSG_NR)
		PE_EVT_INFO("%s\n", pd_hw_msg_name[msg]);
}
...
static inline void print_pe_msg_event(struct tcpc_device *tcpc, u8 msg)
{
	if (msg < PD_PE_MSG_NR)
		PE_EVT_INFO("%s\n", pd_pe_msg_name[msg]);
}
...
static inline void print_dpm_msg_event(struct tcpc_device *tcpc, u8 msg)
{
...
static inline void print_tcp_event(struct tcpc_device *tcpc, u8 msg)
{
...
#if PE_INFO_ENABLE
	u8 __maybe_unused event_type = pd_event->event_type;
	u8 __maybe_unused msg_id = pd_get_msg_hdr_id(pd_port);
	u8 __maybe_unused msg_type = pd_event->msg;
#endif
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
static inline u8 pe_get_startup_state(struct pd_port *pd_port, struct pd_event *pd_event)
{
...
static inline u8 pe_check_trap_in_idle_state(struct pd_port *pd_port, struct pd_event *pd_event)
{
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
