...
static bool fts_fwupg_check_fw_valid(void)
{
	...
	ret = fts_wait_tp_to_valid(true);
	if (ret < 0) {
		FTS_DEBUG("tp fw invaild");
		return false;
	}

	FTS_DEBUG("tp fw valid");
	return true;
}
...
static int fts_lic_get_vid_in_tp(u16 *vid)
{
	...
	if (fts_data->ic_info.is_incell) {
		...
		if (ret < 0) {
			FTS_ERROR("read mid from tp fail");
			return ret;
		}
	}
...
static int fts_param_get_ver_in_host(struct fts_upgrade *upg, u8 *ver)
{
	...
	FTS_DEBUG("fw paramcfg version offset:%x",
...
static bool fts_fwupg_need_upgrade(struct fts_upgrade *upg)
{
	...
	if (fwvalid) {
		...
		FTS_DEBUG("fw version in tp:%x, host:%x",
...
static int fts_fwupg_do_param_upgrade(struct fts_upgrade *upg)
{
	...
	if (ret == 1) {
		FTS_DEBUG("force upgrade fw app");
		return fts_fwupg_do_fw_upgrade(upg);
	}

	if (ret == 2) {
		FTS_DEBUG("upgrade param area");
		...
		FTS_DEBUG("upgrade to fw param version %02x", ver);
		return 0;
	}
...
int fts_fwupg_upgrade(struct fts_upgrade *upg)
{
	...
	do {
		upgrade_count++;
		FTS_DEBUG("upgrade attempt %d", upgrade_count);
		...
		for (i = 0; i < func_count; i++) {
			...
			for (j = 0; j < FTX_MAX_COMPATIBLE_TYPE; j++) {
				if (func->ctype[j] == 0) {
					...
					FTS_DEBUG("match upgrade, ,type:%x", (int)func->ctype[j]);
					fwupgrade->func = func;
				}
			}
		}
