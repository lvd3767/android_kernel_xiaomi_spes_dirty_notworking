					   ...
					   struct pd_battery_info *bat_info)
{
	...
	ret = of_property_read_u32(sub, "bat,vid", &vid);
	if (ret < 0) {
		pr_err("get pd vid fail\n");
		vid = PD_IDH_VID(pd_port->id_vdos[0]);
	}

	ret = of_property_read_u32(sub, "bat,pid", &pid);
	if (ret < 0) {
	...
	strscpy(mfrs_info->mfrs_string, mstring, sizeof(mfrs_info->mfrs_string));
#endif	/* CONFIG_USB_PD_REV30_MFRS_INFO_LOCAL */
...
static inline int pd_parse_pdata_bats(struct pd_port *pd_port, struct device_node *np)
{
	...
	pd_port->fix_bat_info = devm_kcalloc(&pd_port->tcpc->dev,
					     pd_port->bat_nr,
					 ...
					 struct pd_country_authority *country_info)
{
	...
	country_info->data = devm_kcalloc(&pd_port->tcpc->dev,
					  country_info->len,
					  sizeof(*country_info->data),
					  GFP_KERNEL);
...
static inline int pd_parse_pdata_countries(struct pd_port *pd_port, struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "pd,country_nr", &val);
	if (ret < 0) {
		pr_err("get country nr fail\n");
		pd_port->country_nr = 0;
		return 0;
	}
	pd_port->country_nr = (u8)val;

	pr_info("Country NR = %d\n", pd_port->country_nr);
	pd_port->country_info = devm_kcalloc(&pd_port->tcpc->dev,
					     pd_port->country_nr,
...
static inline void pd_parse_pdata_mfrs(struct pd_port *pd_port, struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "pd,vid", &vid);
	if (ret < 0) {
		pr_err("get pd vid fail\n");
		vid = PD_IDH_VID(pd_port->id_vdos[0]);
	}

	ret = of_property_read_u32(np, "pd,pid", &pid);
	if (ret < 0) {
		pr_err("get pd pid fail\n");
		pid = PD_PRODUCT_PID(pd_port->id_vdos[2]);
	}
...
static int pd_parse_pdata(struct pd_port *pd_port)
{
	...
	if (np) {
		ret = of_property_read_u32(np, "pd,source-pdo-size", &val);
		if (ret < 0)
			pr_err("get source pdo size fail\n");
		else
			pd_port->local_src_cap_default.nr = (u8)val;

		ret = of_property_read_u32_array(np, "pd,source-pdo-data",
						 pd_port->local_src_cap_default.pdos,
		...
		for (i = 0; i < pd_port->local_src_cap_default.nr; i++)
			pr_info("%d: 0x%08x\n", i, pd_port->local_src_cap_default.pdos[i]);

		ret = of_property_read_u32(np, "pd,sink-pdo-size", &val);
		if (ret < 0)
			pr_err("get sink pdo size fail\n");
		else
			pd_port->local_snk_cap.nr = (u8)val;

		ret = of_property_read_u32_array(np, "pd,sink-pdo-data",
						 pd_port->local_snk_cap.pdos,
		...
		for (i = 0; i < pd_port->local_snk_cap.nr; i++) {
			pr_info("%d: 0x%08x\n", i, pd_port->local_snk_cap.pdos[i]);
		...
		ret = of_property_read_u32(np, "pd,id-vdo-size", &val);
		if (ret < 0)
			pr_err("get id vdo size fail\n");
		else
			pd_port->id_vdo_nr = (u8)val;

		ret = of_property_read_u32_array(np, "pd,id-vdo-data",
						 pd_port->id_vdos, pd_port->id_vdo_nr);
		if (ret < 0)
...
static void pd_core_power_flags_init(struct pd_port *pd_port)
{
	...
	if (!pd_port || !pd_port->tcpc || !pd_port->tcpc->dev.of_node) {
		pr_err("invalid pd_port or of_node\n");
		return;
	}
...
int pd_core_init(struct tcpc_device *tcpc)
{
	...
	if (ret < 0) {
		pr_err("pd_parse_pdata failed (%d)\n", ret);
		goto out;
	}
...
u32 pd_reset_pdo_power(struct tcpc_device *tcpc,
		       u32 pdo, u32 imax)
{
	u32 ioper;
...
int pd_send_svdm_request(struct pd_port *pd_port,
			 ...
			 u8 obj_pos, u8 cnt, u32 *data_obj, u32 timer_id)
{
