...
{
	bool find_cap = false;
	u8 sel = pd_port->pe_data.local_selected_cap;
				   ...
				   struct dpm_rdo_info_t *req_info)
{
	...
	if (pd_event_data_msg_match(pd_event, PD_DATA_SOURCE_CAP) &&
		...
		if (dpm_build_request_info_with_new_src_cap(pd_port,
							    req_info, src_cap, charging_policy))
			return true;
	}
...
static inline void dpm_dfp_update_partner_id(struct pd_port *pd_port, u32 *payload)
{
...
static inline void dpm_dfp_update_svid_data_exist(struct pd_port *pd_port, u16 svid)
{
	u8 k;
	struct svdm_svid_data *svid_data;
	struct tcpc_device __maybe_unused *tcpc = pd_port->tcpc;
...
{
	const u32 reaction_clear = DPM_REACTION_GET_SINK_CAP
...
{
	u32 reaction_clear = DPM_REACTION_GET_SOURCE_CAP
		| DPM_REACTION_ATTEMPT_GET_FLAG;
	struct pd_event *pd_event = pd_get_curr_pd_event(pd_port);
	struct pd_port_power_caps *src_cap = &pd_port->pe_data.remote_src_cap;
...
int pd_dpm_core_init(struct pd_port *pd_port)
{
	...
	pd_port->svid_data = devm_kzalloc(&tcpc->dev,
					  sizeof(struct svdm_svid_data) * svid_ops_nr,
					  GFP_KERNEL);
