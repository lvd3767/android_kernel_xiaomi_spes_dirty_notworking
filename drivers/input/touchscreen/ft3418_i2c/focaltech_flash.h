...
struct upgrade_func {
	...
	u8 *pramboot;
	u32 pb_length;
	int (*init)(u8 *fw, u32 fw_length);
	int (*write_pramboot_private)(void);
	int (*upgrade)(u8 *fw, u32 fw_length);
	int (*get_hlic_ver)(u8 *lic);
	int (*lic_upgrade)(u8 *lic, u32 lic_length);
	int (*param_upgrade)(u8 *fw, u32 fw_length);
	int (*force_upgrade)(u8 *fw, u32 fw_length);
};
