...
static bool pd_make_vdm_state_transit(struct pd_port *pd_port, u8 transit_type)
{
	...
	u8 vdm_cmdt;
	const struct vdm_state_transition *state_transition;
	u8 nr_transition = ARRAY_SIZE(pe_vdm_state_reactions);
	...
	for (i = 0; i < nr_transition; i++) {
		...
		if (!pe_check_vdm_state_transit_valid(pd_port,
						      transit_type,
...
static inline const char *assign_vdm_cmd_name(u8 cmd)
{
...
static inline const char *assign_vdm_dp_cmd_name(u8 cmd)
{
...
const u8 tcp_vdm_evt_init_state[] = {
...
static inline u32 tcpc_update_bits(u32 var, u32 update, u32 mask)
{
...
static inline bool pd_check_tcp_msg_valid(struct pd_port *pd_port, u8 new_state)
{
...
static inline bool pd_process_tcp_msg(struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	if (!pd_check_pe_state_ready(pd_port)) {
		pd_notify_tcp_event_1st_result(pd_port,
