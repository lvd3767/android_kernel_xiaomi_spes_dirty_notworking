...
{
	struct dual_role_phy_instance *dual_role = container_of(work,
			struct dual_role_phy_instance, changed_work);
...
{
	struct dual_role_phy_instance *dual_role = container_of(dev,
			struct dual_role_phy_instance, dev);
					...
					*dual_role)
{
	...
	rc = devres_release(dev,
			    devm_dual_role_release,
			    devm_dual_role_match,
			    dual_role);
	WARN_ON(rc);
}
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	if (off == DUAL_ROLE_PROP_SUPPORTED_MODES) {
		...
		BUILD_BUG_ON(ARRAY_SIZE(pr_text) != DUAL_ROLE_PROP_PR_TOTAL);
		...
		BUILD_BUG_ON(ARRAY_SIZE(dr_text) != DUAL_ROLE_PROP_DR_TOTAL);
...
int dual_role_uevent(struct device *dev, struct kobj_uevent_env *env)
{
	...
	if (dual_role->desc->name && dual_role->desc->name[0]) {
		...
		ret = add_uevent_var(env, "DUAL_ROLE_NAME=%s",
				     dual_role->desc->name);
	} else {
	...
	if (!dual_role->desc->properties ||
	    dual_role->desc->num_properties <= 0) {
	...
	if (dual_role->desc->num_properties > 1024) {
		dev_dbg(dev, "%s: num_properties too large: %zu\n",
			__func__, dual_role->desc->num_properties);
		return -EINVAL;
	}
