...
int fts_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
	...
	proc->proc_entry = proc_create(PROC_NAME, 0600, NULL, &fts_proc_fops);
	if (!proc->proc_entry) {
	...
	if (rw_op.type == RWREG_OP_READ) {
		if (rw_op.len == 1) {
			...
			if (rw_op.res < 0) {
				...
				FTS_DEBUG("read 0x%02x, %d bytes successful",
					  rw_op.reg, rw_op.len);
				rw_op.res = 0;
				rw_op.dirty = true; // Mark result available
			}
		}
	} else if (rw_op.type == RWREG_OP_WRITE) {
		if (rw_op.len == 1) {
			u8 reg = rw_op.reg & 0xFF;
				   ...
				   const char *buf, size_t count)
{
	...
	if (ret) {
		FTS_ERROR("invalid log level input, ret: %d", ret);
		return ret;
	}
	mutex_lock(&input_dev->mutex);
...
/* get the fw version: cat fts_fw_version */
...
/* upgrade from fw bin file: echo "*.bin" > fts_upgrade_bin */
