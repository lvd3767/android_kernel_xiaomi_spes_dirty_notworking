...
u8 tcpm_inquire_cable_current(struct tcpc_device *tcpc)
{
	struct pd_port *pd_port = &tcpc->pd_port;
...
int tcpm_inquire_cable_inform(struct tcpc_device *tcpc, u32 *vdos)
{
	...
	mutex_lock(&pd_port->pd_lock);
	if (pd_port->pe_data.power_cable_present)
		memcpy(vdos, pd_port->pe_data.cable_vdos, sizeof(u32) * VDO_MAX_NR);
	else
...
int tcpm_inquire_pd_partner_inform(struct tcpc_device *tcpc, u32 *vdos)
{
	...
	if (pd_port->pe_data.partner_id_present)
		memcpy(vdos, pd_port->pe_data.partner_vdos, sizeof(u32) * VDO_MAX_NR);
	else
				  ...
				  struct tcpm_svid_list *list)
{
	...
	if (svdm_list->cnt) {
		...
		memcpy(list->svids, svdm_list->svids, sizeof(u16) * svdm_list->cnt);
	} else {
		ret = TCPM_ERROR_NO_PARTNER_INFORM;
	}
	mutex_unlock(&pd_port->pd_lock);
				  ...
				  u16 svid, struct tcpm_mode_list *list)
{
	...
	if (svid_data->remote_mode.mode_cnt) {
		...
		memcpy(list->modes, svid_data->remote_mode.mode_vdo, sizeof(u32) * list->cnt);
	} else {
		ret = TCPM_ERROR_NO_PARTNER_INFORM;
	}
	mutex_unlock(&pd_port->pd_lock);
			       ...
			       struct tcpm_power_cap *cap)
{
	...
	if (pd_port->pe_data.remote_src_cap.nr) {
		...
		memcpy(cap->pdos, pd_port->pe_data.remote_src_cap.pdos, sizeof(u32) * cap->cnt);
	} else {
			     ...
			     struct tcpm_power_cap *cap)
{
	...
	if (pd_port->pe_data.remote_snk_cap.nr) {
		...
		memcpy(cap->pdos, pd_port->pe_data.remote_snk_cap.pdos, sizeof(u32) * cap->cnt);
	} else {
...
int tcpm_dpm_pd_power_swap(struct tcpc_device *tcpc,
			   u8 role, const struct tcp_dpm_event_cb_data *cb_data)
{
...
int tcpm_dpm_pd_alert(struct tcpc_device *tcpc,
		      u32 ado, const struct tcp_dpm_event_cb_data *cb_data)
{
	struct tcp_dpm_event tcp_event = {
...
int tcpm_dpm_vdm_discover_mode(struct tcpc_device *tcpc,
			       u16 svid, const struct tcp_dpm_event_cb_data *cb_data)
{
	struct tcp_dpm_event tcp_event = {
			     ...
			     const struct tcp_dpm_event_cb_data *cb_data)
{
	...
	if (ret == TCP_DPM_RET_SUCCESS &&
		...
		memcpy(vdm_data->vdos,
		       tcpc->pd_port.uvdm_data, sizeof(u32) * vdm_data->cnt);
		mutex_unlock(&pd_port->pd_lock);
	}
...
int tcpm_set_pd_charging_policy(struct tcpc_device *tcpc,
				u8 policy, const struct tcp_dpm_event_cb_data *cb_data)
{
	struct tcp_dpm_event tcp_event = {
...
static inline u16 tcpm_calc_battery_cap(struct pd_battery_info *battery_info, u16 soc)
{
	u16 wh;
...
int tcpm_update_pd_status_temp(struct tcpc_device *tcpc,
			       enum pd_present_temperature_flag ptf_new, u8 temperature)
{
...
static inline int __tcpm_dpm_wait_bk_event(struct pd_port *pd_port, u32 tout_ms)
{
...
int tcpm_dpm_wait_bk_event(struct pd_port *pd_port, u32 tout_ms)
{
...
{
	int ret;
	u8 retry = CONFIG_USB_PD_TCPM_CB_RETRY;
	struct pd_port *pd_port = &tcpc->pd_port;
