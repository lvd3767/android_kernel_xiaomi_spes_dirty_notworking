...
static void tcp_notify_func(struct work_struct *work)
{
	...
	u8 type = tn_work->type;
	u8 state = tn_work->state;
...
{
	PD_BUG_ON(!tcpc->ops->alert_status_clear);
...
{
	PD_BUG_ON(!tcpc->ops->fault_status_clear);
...
{
	PD_BUG_ON(!tcpc->ops->get_alert_mask);
...
{
	PD_BUG_ON(!tcpc->ops->get_alert_status);
...
int tcpci_get_power_status(struct tcpc_device *tcpc, u16 *pw_status)
{
	...
	PD_BUG_ON(!tcpc->ops->get_power_status);
...
{
	PD_BUG_ON(!tcpc->ops->set_polarity);
...
int tcpci_set_msg_header(struct tcpc_device *tcpc, u8 power_role, u8 data_role)
{
	PD_BUG_ON(!tcpc->ops->set_msg_header);
...
{
	PD_BUG_ON(!tcpc->ops->set_rx_enable);
...
{
	PD_BUG_ON(!tcpc->ops->get_message);
...
{
	PD_BUG_ON(!tcpc->ops->set_bist_carrier_mode);
