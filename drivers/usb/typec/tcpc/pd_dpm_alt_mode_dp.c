...
static inline bool dp_update_dp_connected_one(struct pd_port *pd_port,
					      u32 dp_connected, u32 dp_local_connected)
{
...
static inline bool dp_update_dp_connected_both(struct pd_port *pd_port,
					       u32 dp_local_connected, bool both_connected_valid)
{
	struct dp_data *dp_data = pd_get_dp_data(pd_port);
...
static inline bool is_dp_v1_cap_valid(u32 dp_cap)
{
...
static inline int eval_dp_match_score(u32 local_mode,
				      u32 remote_mode,
				      u32 *local_dp_config,
				      u32 *remote_dp_config)
{
...
static inline u8 dp_dfp_u_select_mode(struct pd_port *pd_port,
			...
			struct svdm_svid_data *svid_data)
{
	...
	if (!pd_port || !pd_port->tcpc) {
		pr_err("invalid pd_port or tcpc\n");
		return false;
	}

	if (!pd_port->tcpc->dev.of_node) {
		pr_err("tcpc->dev.of_node is NULL\n");
		return false;
	}
